...
| `destinationScale`   | Integer        | Scale of the amounts on the destination ledger. Used to format the `destinationAmount` in the return value. This SHOULD be reported by the ledger.<br/>**Note:** This must be communicated in the Application Layer protocol. If this value is incorrect, the result will be off by several orders of magnitude. |
